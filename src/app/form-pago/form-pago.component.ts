          ...
          handler: (data) => {
            if (data.length > 0) {
              ...
              if (Number(upgrade.rd_calc_type) == 2) {
                ...
                for (let i = 0; i < this.arrUpgradesAplicados.length; i++) {
                  auxPrec = 0;
                  for (let a = 0; a < this.arrUpgradesAplicados[i].pasajero.length; a++) {

                    if (this.arrUpgradesAplicados[i].tipo_calculo == 2) {
                      ///aplica para upgrade con limite de edad activo
                      if (upgrade.limiteage.toUpperCase() == 'Y') {
                        //Selecciona el precio de mayores y aplica upgrade
                        if (this.users.controls[this.arrUpgradesAplicados[i].pasajero[a]].value.edad > this.dataPlan.normal_age && this.users.controls[this.arrUpgradesAplicados[i].pasajero[a]].value.edad <= this.dataPlan.max_age && (this.users.controls[this.arrUpgradesAplicados[i].pasajero[a]].value.edad >= upgrade.limiteedadmin) && (this.users.controls[this.arrUpgradesAplicados[i].pasajero[a]].value.edad <= upgrade.limiteedadmax)) {
                          auxPrec += Number((Number(this.arrUpgradesAplicados[i].type_raider) == 1) ? Number(this.arrUpgradesAplicados[i].monto_upgrade) : (Number(this.dataPlan.valorMayor) && (Number(this.dataPlan.numero_mayores > 0)) && (Number(this.dataPlan.subTotalMayor) > 0)) ? (Number(this.dataPlan.valorMayor) * Number(this.arrUpgradesAplicados[i].monto_upgrade) / 100) : (Number(this.dataPlan.valorMenor)) * Number(this.arrUpgradesAplicados[i].monto_upgrade) / 100);
                          this.arrUpgradesAplicados[i].monto_aplicado = this.formatNumberDecimal(Number(auxPrec));
                          console.log('agrega a pasajero Mayor ', auxPrec);
                        }

                        //Selecciona el precio de menores y aplica upgrade
                        else {
                          auxPrec += Number((Number(this.arrUpgradesAplicados[i].type_raider) == 1) ? Number(this.arrUpgradesAplicados[i].monto_upgrade) : ((Number(this.dataPlan.valorMenor) * Number(this.arrUpgradesAplicados[i].monto_upgrade)) / 100));
                          this.arrUpgradesAplicados[i].monto_aplicado = this.formatNumberDecimal(Number(auxPrec));
                          console.log('agrega a pasajero Menor ', auxPrec);
                        }
                      } else {
                        /////aplica para todos los pasajeros 
                        if (this.users.controls[this.arrUpgradesAplicados[i].pasajero[a]].value.edad > this.dataPlan.normal_age && this.users.controls[this.arrUpgradesAplicados[i].pasajero[a]].value.edad <= this.dataPlan.max_age) {
                          auxPrec += Number((Number(this.arrUpgradesAplicados[i].type_raider) == 1) ? Number(this.arrUpgradesAplicados[i].monto_upgrade) : (Number(this.dataPlan.valorMayor) && (Number(this.dataPlan.numero_mayores > 0)) && (Number(this.dataPlan.subTotalMayor) > 0)) ? (Number(this.dataPlan.valorMayor) * Number(this.arrUpgradesAplicados[i].monto_upgrade) / 100) : (Number(this.dataPlan.valorMenor)) * Number(this.arrUpgradesAplicados[i].monto_upgrade) / 100);
                          this.arrUpgradesAplicados[i].monto_aplicado = this.formatNumberDecimal(Number(auxPrec));
                          console.log('agrega a pasajero Mayor ', auxPrec);
                        }
              ...
              if (Number(upgrade.rd_calc_type) == 5) {
                ...
                for (let i = 0; i < this.arrUpgradesAplicados.length; i++) {
                  /////aplica para todos los pasajeros 
                  if (this.arrUpgradesAplicados[i].tipo_calculo == 5) {
                    auxPrec = Number((Number(this.arrUpgradesAplicados[i].type_raider) == 1) ? Number((this.arrUpgradesAplicados[i].monto_upgrade * this.diasViaje) * data.length) : Number((((this.arrUpgradesAplicados[i].monto_upgrade / 100) * this.dataPlan.USDTotal) * this.diasViaje) * data.length));
                    this.arrUpgradesAplicados[i].monto_aplicado = this.formatNumberDecimal(Number(auxPrec));
                    console.log('agrega raider a  ', data.length, 'raider tipo', upgrade.rd_calc_type);
                  }
                }
